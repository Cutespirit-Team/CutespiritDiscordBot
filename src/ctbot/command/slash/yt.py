...
class SlashYT(commands.Cog):
    ...
    async def play(self, ctx, url: str=None):
        ...
        if not ctx.author.voice:
            await ctx.send('無法取得語音頻道', hidden=True)
            return
        ...
        if url:
            if self.player.voice_channel == ctx.author.voice.channel:
                ...
                await ctx.send('已加入清單', hidden=True)
            else:
                await ctx.send('語音頻道與機器人不符, 無權添加', hidden=True)
        else:
            await ctx.send('播放音樂', hidden=True)
